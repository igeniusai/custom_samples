...
def _render_verdict_history_ui(title: str, data_path: str) -> str:
    tpl = string.Template((_TEMPLATES_DIR / "verdict_history_ui.html").read_text())
    return tpl.substitute(title=title, data_path=data_path)
...
def _render_admin_ui(
    ...
    tpl = string.Template((_TEMPLATES_DIR / "admin_ui.html").read_text())
